...
long  			NumThreads,PrThreads;  		// Total number of threads working in parallel
...
void CopyDoubleToBMP(unsigned char** UCimg, double** Dimg, unsigned char Invert)
{
    int i,j,k;
	unsigned char *UC;
	double *D;
	...
	for(i=0; i<ip.Vpixels; i++){
		UC=UCimg[i];
		D=Dimg[i];
		for(j=0, k=0; k<ip.Hpixels; j+=3, k++){
			U=(unsigned char) D[k];
			if(Invert) U=255-U;     // Invert the pixel if required
			UC[j]=U;
			UC[j+1]=U;
			UC[j+2]=U;
		} 
...
int main(int argc, char** argv)
{
	...
	/////////////////////////////////////////////////////////////////////////////////////////	
	// This extra code writes the Gauss and Sobel images to disk also
	CopyDoubleToBMP(CopyImage,GaussImage,0);  	// Convert the GaussImage[] to CopyImage[]
	WriteBMP(CopyImage, "Gauss.bmp");  			// Write GaussImage[] to disk
	printf("Gauss Image of %s written to Gauss.BMP\n",argv[2]);
	CopyDoubleToBMP(CopyImage,Gradient,1);   	// Convert the Gradient[] to CopyImage[]. Invert
	WriteBMP(CopyImage, "Gradient.bmp");  		// Write Gradient[] to disk
	printf("Gradient Image of %s written to Gradient.BMP\n",argv[2]);
/////////////////////////////////////////////////////////////////////////////////////////	
